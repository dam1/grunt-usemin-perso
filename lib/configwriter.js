...
//  if (grunt.config(name).generated.files[1] != null) {
//   var aa = grunt.config('concat').generated.files;
 var aa = config.concat.generated.files;
//    var aa = grunt.config('concat').generated.files;
 ...
 for (var j = 0; j < aa.length; j++) {
  ...
  for (var i = 0; i < bb.length; i++) {
   ...
   if (bb[i].indexOf('http') != -1) {
//      console.log(aa[i]);

    var url =  'http'+ bb[i].split('http')[1];
//   url =  url.replace(':/', '://');
...
//     console.log(host[1]);
    ...
    config.concat.generated.files[j].src[i] ="grunt-download/aa" + i + ".js";


    var result = fs.writeFileSync("./grunt-download/aa" + i + ".js", response.body);

    console.log('file '+url+" download and saved");
   }
